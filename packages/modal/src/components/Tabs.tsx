...
import { SearchedTab, EllipsisText, HStack, scrollbarStyle } from '../styles';
...
type PanelProps<T> = {
  ...
  setScrollingState: React.Dispatch<
    React.SetStateAction<'arrows' | 'mouse' | undefined>
  >;
};
...
const BlockView = styled(({ title, url, ...rest }: BlockedViewProps) => (
  <div {...rest}>
    <Text>{title}</Text>
    <Text textAlign="right" isUrl>
      {url}
    ...
    <Container data-testid="tab-master-tabs" isScrollable={!headingTitle}>
      ...
      {tabs.map((tab, i) => (
        <SearchedTab
          ...
          key={tab.id}
          data-testselected={
            selectedTabId === (tab[clickCallbackField] as unknown as string)
              ? `selected-${i}`
              : ''
          }
          isSelected={
          ...
          onClick={() =>
            onTabClicked(tab[clickCallbackField as unknown as string])
          }
          onMouseEnter={() => {
        ...
        >
          ...
          {expandedTabIds.includes(tab.id) || !isMinimal ? (
            <BlockView
              data-testexpanded={
                expandedTabIds.includes(tab.id) ? `expanded-${i}` : ''
              }
          ...
          <IconContainer
            isAtTop={!isMinimal || expandedTabIds.includes(tab.id)}
          >
            {tab.action === 'open' ? <ExternalIcon /> : <RandomIcon />}
          </IconContainer>
        </SearchedTab>
      ))}
    </Container>
